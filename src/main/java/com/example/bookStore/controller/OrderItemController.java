...
public class OrderItemController {
    ...
    public ResponseEntity<List<OrderItemDto>> getOrderItemsByBookId(@PathVariable Long productId) {
        List<OrderItem> orderItems = orderItemService.findAllByBookId(productId);
        return ResponseEntity.ok(orderItems.stream().map(orderItemService::convertToDto).toList());
    }
